...
function run_bazel_build() {
  ...
  echo bazel build --copt="-Wno-sign-compare -m32" ${CMDLINE_OPTIONS} ${job_args} -- ${formatted_targets}
  bazel build --copt="-Wno-sign-compare -m32" ${CMDLINE_OPTIONS} ${job_args} -- ${formatted_targets}
}
